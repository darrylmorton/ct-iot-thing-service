...
function check_version_greater () {
  ...
  # check if the top sorted version equals the current version. If so we have a new version
...
if check_version_greater "$CURRENT_VERSION" "$PUBLISHED_VERSIONS"; then
  echo "##[{name}={VERSION;]v$CURRENT_VERSION}"
  echo "##[{name}={BUILD_DATE;]$(date -u +'%Y-%m-%dT%H:%M:%SZ')}"
  echo "##[{name}={IS_NEW_VERSION;]true}"
  if [[ $CURRENT_VERSION =~ [-] ]]; then
    echo "##[{name}={IS_PRERELEASE;]true}"
    echo "##[{name}={NPM_RELEASE_TAG;]next}"
  else
    echo "##[{name}={IS_PRERELEASE;]false}"
    echo "##[{name}={NPM_RELEASE_TAG;]latest}"
  fi
else
  echo "##[{name}={IS_NEW_VERSION;]false}"
fi
